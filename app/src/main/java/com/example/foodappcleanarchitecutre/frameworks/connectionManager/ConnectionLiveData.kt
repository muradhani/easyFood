...
class ConnectionLiveData(context: Context) : LiveData<Boolean>() {
    ...
    companion object {
        fun create(context: Context): LiveData<Boolean> {
            return ConnectionLiveData(context)
        }
    }
    init {
        // Perform the initial check when the object is created
        checkInternetStatus()
    }
    private lateinit var networkCallback: ConnectivityManager.NetworkCallback
    private val cm = context.getSystemService(CONNECTIVITY_SERVICE) as ConnectivityManager
    private val validNetworks: MutableSet<Network> = HashSet()
    private var isInitialCheckPerformed = false

    private fun checkInternetStatus() {
        if (cm != null) {
            CoroutineScope(Dispatchers.IO).launch {
                val hasInternet = DoesNetworkHaveInternet.execute(SocketFactory.getDefault())
                withContext(Dispatchers.Main) {
                    if (hasInternet) {
                        Log.d(TAG, "Initial internet check successful.")
                        postValue(true)
                    } else {
                        Log.d(TAG, "Initial internet check failed.")
                        postValue(false)
                    }
            ...
            Log.e(TAG, "ConnectivityManager is null. Unable to perform initial internet check.")
        }
    }
    ...
    override fun onActive() {
        if (!isInitialCheckPerformed) {
            // If the initial check hasn't been performed, perform it now
            checkInternetStatus()
        }
